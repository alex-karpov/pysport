...
- [x] 13.5 **Fixed** (pre-existing, surfaced while checking the CI artifacts). The MSI has no `Upgrade` table and `bdist_msi_options` sets no `upgrade_code`, so every build gets a fresh `ProductCode` and Windows treats each one as an unrelated product: installing 1.8.0 over 1.8.0b2 adds a second entry in *Программы и компоненты* pointing at the same directory, and uninstalling either one takes the shared files with it. `ProductVersion` compounds it — msilib requires a numeric version, so `1.8.0b2` is stored as `1.8.0` (the real value is kept in `DistVersion`). `bdist_msi_options` now sets `upgrade_code` to `{D652DEE1-13E6-4D7A-B8FC-334FF475E5FD}` — a random UUID4, fixed for good; a distinct GUID from the Inno `AppId`, which identifies the same product in a registry the two installers never share. `ProductCode` stays random per build, which is what a major upgrade requires. Verified in the rebuilt package: `FindRelatedProducts` at 200, `RemoveExistingProducts` at 1450, both `Upgrade` rows written, `LockPermissions` untouched.
  **Two limits remain, both inherent.** A copy installed from an MSI built *before* this commit carries no `UpgradeCode`, so `FindRelatedProducts` cannot see it — the 1.8.0b2 artifacts already handed out must be uninstalled by hand once. The second limit is not the one first recorded here. `ProductVersion` is truncated to `1.8.0`, but that does not block the upgrade: `Attributes = 513` is `msidbUpgradeAttributesVersionMaxInclusive | MigrateFeatures`, so the removal row covers everything up to **and including** 1.8.0, and the final 1.8.0 does supersede an installed 1.8.0b2. What truncation actually costs is that b1, b2 and the final build are indistinguishable to the installer, and the second row (`257` = `VersionMinInclusive | MigrateFeatures`) carries no `OnlyDetect` flag while `LaunchCondition` stays empty — so newer versions are removed rather than merely detected, and installing an older build over a newer one silently downgrades it

## 14. Forward compatibility with cx_Freeze 8

`pyproject.toml` already declares `cx_Freeze>=8; python_version >= '3.14'`, but that path had never been exercised. Verified by installing `cx_Freeze==8.7.0` into a throwaway 3.13 environment and running `bdist_msi` on a trivial payload with `builder.py`'s exact option dictionaries — 8.6+ is a pure `py3` wheel, so its `bdist_msi` is byte-identical for 3.13 and 3.14.

- [x] 14.1 `base="Win32GUI"` is a hard failure under 8.x: the bases were renamed and the sets do not overlap (6.15 ships `console`/`Win32GUI`/`Win32Service`, `freeze-core` 0.7.2 ships `console`/`gui`/`gui_dgpu`/`service`; only C sources remain under `freeze_core/legacy/`). Since `builder.py` has to serve both, the name is now chosen from `cx_Freeze.__version__`
- [x] 14.2 8.x derives the installer file name from `ProductVersion`, which MSI forces to be numeric, so `v1.8.0b2` would ship as `SportOrg-1.8.0-win64.msi` and collide with the eventual 1.8.0. Pinned with the new `output_name` option, reproducing what 6.x produced. `setuptools` resolves the project name from `pyproject.toml`, not the `setup()` argument, so the historical name is `sportorg-1.8.0b2-win64.msi` — confirmed with `Distribution.get_name()`
- [x] 14.3 Everything else in `bdist_msi_options` transfers verbatim: `all_users` (still `False` by default), `upgrade_code`, `initial_target_dir` (8.x now also defaults to `ProgramFiles64Folder` on x64), and arbitrary table rows through `data`. The generated `Directory`, `CreateFolder`, `LockPermissions`, `Shortcut`, `Upgrade`, `Property`, `CustomAction` and sequence rows match the 6.15 output exactly. The `TARGETDIR` component still exists, so the shortcut's component reference stays valid, even though executables now get their own component. All eight `build_exe` options are unchanged, and `Executable(icon=…, copyright=…)` is still accepted
- [x] 14.4 Both paths re-verified after the fix: `builder.py` loads under 6.15.16/Python 3.8 and picks `Win32GUI`; the synthetic 8.7.0 build emits `sportorg-1.8.0b2-win64.msi` with identical tables
- [ ] 14.5 **Not covered:** the synthetic build does not freeze the real application. PySide6 hooks, `zip_include_packages`, `pywinusb`, the Rust extension and `include_files` for `sportorg/data` are untested under 8.x, which rewrote the hook system. `ProductVersion` truncation is unchanged, so both upgrade-code limitations in 13.5 survive the move
