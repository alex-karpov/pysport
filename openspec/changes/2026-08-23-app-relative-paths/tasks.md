...
- [ ] 13.5 **Pre-existing, needs a decision before the release.** The MSI has no `Upgrade` table and `bdist_msi_options` sets no `upgrade_code`, so every build gets a fresh `ProductCode` and Windows treats each one as an unrelated product: installing 1.8.0 over 1.8.0b2 adds a second entry in *Программы и компоненты* pointing at the same directory, and uninstalling either one takes the shared files with it. `ProductVersion` compounds it — msilib requires a numeric version, so `1.8.0b2` is stored as `1.8.0` (the real value is kept in `DistVersion`). Fixed by one permanent GUID in `bdist_msi_options`, but the GUID is product identity and must never change afterwards, so it is the maintainer's call, not this change's
