# race-short-title Specification

## Purpose

Provide a short, operator-only label per race (`short_title`) used to identify the race in the application UI (window title, multi-day race selectors) without affecting printed protocols. The label is editable via the Event Properties dialog and persists with the race data.
