...
+ В релизы вместе с MSI теперь входят инсталлятор Inno Setup и portable-архив, инсталляторы создают `{app}\data` и `{app}\logs` и выдают права на запись
+ Оба инсталлятора теперь предлагают 64-битный каталог `Program Files`; инсталлятор Inno Setup для 64-битной сборки предлагал `Program Files (x86)`
+ В MSI добавлен upgrade code — новая версия заменяет установленную, а не встаёт второй записью рядом
