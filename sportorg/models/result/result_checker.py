...
class ResultChecker:
    ...
    def calculate_rogaine_score(result: Result, allow_duplicates: bool = False) -> int:
        ...
        """
        ROGAINE_SETTINGS = settings.SETTINGS.NOVOSIVIRSK_ROGAINE_PAIRS
        NOVOSIBIRSK_VYBOR = ROGAINE_SETTINGS.get("enabled", False)
        if NOVOSIBIRSK_VYBOR:
            person = result.person
            group = person.group if person and person.group else None
            course = group.course if group and group.course else None
            course_name = course.name if course else ""

            """
            Ограничение количества засчитываемых КП.

            "NOVOSIVIRSK_ROGAINE_PAIRS": {
                "enabled": true,
                "limit_num_controls": true
            }
            """
            LIMIT_NUM_CONTROLS = ROGAINE_SETTINGS.get("limit_num_controls", False)

            """
            Список КП, ккоторые на засчитываются при подсчёте очков

            "NOVOSIVIRSK_ROGAINE_PAIRS": {
                "enabled": true,
                "ignored_controls_by_course": {
                        "AB": ["160", "173"],
                        "CD": ["163", "176"]
                }
            """
            entries = ROGAINE_SETTINGS.get("ignored_controls_by_course", {})
            ignored_controls = entries.get(course_name, []) if course_name else []
            ...
            только один КП из связки, он засчитывается как обычный КП (1 балл / стоимость КП). 
            Если между двумя КП из связки отмечен хотя бы один другой КП, связка не засчитываются.
            Бонус к стоимости связки задаётся в настройках (очки за КП1 + очки за КП2 + бонус).
            ...
            """
            pair_entries = ROGAINE_SETTINGS.get("pair_score_bonus", [])
            pairs = {}
            for pair_entry in pair_entries:
                if course_name in pair_entry["course_names"]:
                    pairs[pair_entry["pair1"]] = {
                        "second": pair_entry["pair2"],
                        "bonus": pair_entry["score_bonus"],
                    }

            # Добавить обратный порядок взятия: {"31": "32"} -> {"31": "32", "32": "31"}
            pairs.update(
                {
                    v["second"]: {"second": k, "bonus": v["bonus"]}
        ...
        for cur_split in result.splits:
            ...
            if NOVOSIBIRSK_VYBOR:
                if LIMIT_NUM_CONTROLS:
                    if len(user_array) >= len(course.controls if course else []):
                        continue
                if code not in set(user_array + ignored_controls) or allow_duplicates:
        ...
        if NOVOSIBIRSK_VYBOR:
            ...
            for cur_split in result.splits:
                code = str(cur_split.code)
                if code in pairs and pairs[code]["second"] == prev_code:
                    score += pairs[code]["bonus"]
                    pairs.pop(code)
                    pairs.pop(prev_code)
                prev_code = code
